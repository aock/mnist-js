...
const modelP = tf.loadModel("./w-model.json");
...
c2d.fillStyle = "white";
let prev = null;
canvas.addEventListener("mousedown", ev => {prev = ev;}, false);
canvas.addEventListener("mouseup", ev => {prev = null;}, false);
...
async function predict() {
    ...
    tf.tidy(() => {
        ...
        const input = r.add(g).add(b).cast("float32").
              div(tf.scalar(-1 * 3 * 255)).add(tf.scalar(1)).
              reshape([1, 28, 28, 1]);
